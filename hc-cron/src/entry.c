 ...
 */
entry *
load_entry (FILE * file,
	    void (*error_func) (), struct passwd *pw, char **envp)
{
...
/*#endif*/
   ...
   * '!' just one.
   * '\' allows commands to start with * or !.
   * added by hcl feb98
   */
  Debug (DPARS, ("load_entry()...looking for flags\n"));
  ...
  switch (ch)
    ...
    case '*':
      ...
      Debug (DPARS, ("found a '*', catch up all occurences of this job\n"));
      break;
    case '!':
      e->flags |= (RUN_DOWNTIME | RUN_DOWN_ONCE);
      Debug (DPARS, ("found a '!', catch up one occurence of this job\n"));
      break;
    case '\\':			/* do nothing, next char will start command */
      Debug (DPARS, ("found a '\\', no more flags\n"));
      break;
    default:			/* oops, no flag, we ate one of the command's chars */
      unget_char (ch, file);
      Debug (DPARS, ("no flags found\n"));
    }
...
static char
get_list (bitstr_t * bits,
	  int low, int high, char *names[], int ch, FILE * file)
{
  register int done;
