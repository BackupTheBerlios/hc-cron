...
/* this holds the sorted list of all jobs we have to catch up 
 * added by hcl feb98
 */
typedef struct _list_cu
{
  struct _list_cu *next;	/* links */
  user *cul_u;			/* job description */
  entry *cul_e;
  time_t rtime;			/* time when it occured, for sorting */
}
list_cu;
...
char *env_get __P ((char *, char **)),
  ...
  **env_init __P ((void)),
  **env_copy __P ((char **)), **env_set __P ((char **, char *));
